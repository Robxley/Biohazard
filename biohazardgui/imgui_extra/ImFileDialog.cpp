...
namespace ifd {
	...
	bool PathBox(const char* label, std::filesystem::path& path, char* pathBuffer, ImVec2 size_arg) {
		...
		if (!(*state & 0b001)) {
			...
			for (auto comp : path) {
				std::string section = comp.generic_string();
			...
			if (!anyOtherHC && clicked) {
				strcpy(pathBuffer, path.generic_string().c_str());
				*state |= 0b001;
	...
	FileDialog::FileData::FileData(const std::filesystem::path& path) {
		...
		struct stat attr;
		stat(path.generic_string().c_str(), &attr);
	...
	FileDialog::FileDialog() {
		...
		for (const auto& entry : std::filesystem::directory_iterator("/", ec)) {
			if (std::filesystem::is_directory(entry, ec))
				thisPC->Children.push_back(new FileTreeNode(entry.path().generic_string()));
		}
		m_treeCache.push_back(thisPC);
#endif
	}
	...
	{
		auto itr = std::find(m_favorites.begin(), m_favorites.end(), m_currentDirectory.generic_string());
	...
	void FileDialog::m_select(const std::filesystem::path& path, bool isCtrlDown)
	{
		...
		if (m_selections.size() == 1) {
			std::string filename = m_selections[0].filename().generic_string();
			if (filename.size() == 0)
				filename = m_selections[0].generic_string(); // drive
			...
			for (const auto& sel : m_selections) {
				std::string filename = sel.filename().generic_string();
				if (filename.size() == 0)
					filename = sel.generic_string();
	...
	{
#ifdef _WIN32
		if (m_icons.count(path.generic_string()) > 0)
			return m_icons[path.generic_string()];

		std::string pathU8 = path.generic_string();
	...
	void FileDialog::m_loadPreview()
	{
		for (size_t i = 0; m_previewLoaderRunning && i < m_content.size(); i++) 
		{
			...
			if (data.Path.has_extension() && ImportImage)
			{
				std::string ext = data.Path.extension().generic_string();
				auto is_iconable = [&] {
					if(m_iconableExtensions.empty())
						return true;
					for (const auto& ext_icons : m_iconableExtensions) {
						if (ext == ext_icons)
							return true;
					}
					return false;
				...
				if (is_iconable())
				{
					if (!ImportImage || !ImportImage(data))
						continue;
					...
					assert(data.IconPreviewData != nullptr);
					assert(data.IconPreviewWidth != 0);
					assert(data.IconPreviewHeight != 0);
				}
			}
	...
	void FileDialog::m_setDirectory(const std::filesystem::path& p, bool addHistory)
	{
		...
		if (p.generic_string().size() == 2 && p.generic_string()[1] == ':')
			m_currentDirectory = std::filesystem::path(p.generic_string() + "\\");
#endif
		...
		if (p.generic_string() == "Quick Access") {
		...
		else if (p.generic_string() == "This PC") {
			for (auto& node : m_treeCache) {
				if (node->Path == p)
					for (auto& c : node->Children)
						m_content.push_back(FileData(c->Path));
			...
			if (std::filesystem::exists(m_currentDirectory, ec))
				for (const auto& entry : std::filesystem::directory_iterator(m_currentDirectory, ec)) {
					...
					if (m_searchBuffer[0]) {
						std::string filename = info.Path.generic_string();
					...
					if (!info.IsDirectory && m_type != IFD_DIALOG_DIRECTORY) {
						if (m_filterSelection < m_filterExtensions.size()) {
							...
							if (exts.size() > 0) {
								std::string extension = info.Path.extension().generic_string();
	...
	void FileDialog::m_sortContent(unsigned int column, unsigned int sortDirection)
	{
		...
		if (m_content.size() > 0) {
			...
			auto compareFn = [column, sortDirection](const FileData& left, const FileData& right) -> bool {
				// name
				if (column == 0) {
					std::string lName = left.Path.generic_string();
					std::string rName = right.Path.generic_string();
	...
	void FileDialog::m_renderTree(FileTreeNode* node)
	{
		...
		std::string displayName = node->Path.stem().generic_string();
		if (displayName.size() == 0)
			displayName = node->Path.generic_string();
			...
			if (!node->Read) {
				...
				if (std::filesystem::exists(node->Path, ec))
					for (const auto& entry : std::filesystem::directory_iterator(node->Path, ec)) {
						if (std::filesystem::is_directory(entry, ec))
							node->Children.push_back(new FileTreeNode(entry.path().generic_string()));
					}
				node->Read = true;
			}
		...
		if (m_zoom == 1.0f) {
			if (ImGui::BeginTable("##contentTable", 3, /*ImGuiTableFlags_Resizable |*/ ImGuiTableFlags_Sortable, ImVec2(0, -FLT_MIN))) {
				...
				for (auto& entry : m_content) {
					...
					if (filename.size() == 0)
						filename = entry.Path.generic_string(); // drive
	...
	void FileDialog::m_renderPopups()
	{
		...
		if (ImGui::BeginPopupModal("Are you sure?##delete")) {
			if (m_selectedFileItem >= static_cast<int>(m_content.size()) || m_content.size() == 0)
				...
				ImGui::TextWrapped("Are you sure you want to delete %s?", data.Path.filename().generic_string().c_str());
				if (ImGui::Button("Yes")) {
					std::error_code ec;
	...
	void FileDialog::m_renderFileDialog()
	{
		...
		if (FavoriteButton("##dirfav", std::count(m_favorites.begin(), m_favorites.end(), m_currentDirectory.generic_string()))) {
			if (std::count(m_favorites.begin(), m_favorites.end(), m_currentDirectory.generic_string()))
				RemoveFavorite(m_currentDirectory.generic_string());
			else 
				AddFavorite(m_currentDirectory.generic_string());
		}
		ImGui::SameLine();
		ImGui::PopStyleColor();
